...
function gra() {
    ...
    function update() {
        ...
        let lvl1 = [poziom[0], poziom[1]]
        let lvl2 = [document.getElementById("l1"), document.getElementById("l2")]

        for (let i = 0; i < lvl1.length; i++) {
            lvl2[i].src = vol1 + lvl1[i] + ".png"
        }
        ...
        if (wirusy == 0) {
            ...
            window.addEventListener("keydown", lvlup)
...
function lvlup(event) {
    poziom = parseInt(poziom)
    if (event.ctrlKey) {
        window.removeEventListener("keydown", lvlup)
        poziom++
